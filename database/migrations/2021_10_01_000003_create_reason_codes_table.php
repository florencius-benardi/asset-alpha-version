...
class CreateReasonCodesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create(ReasonCode::ATTR_TABLE, function (Blueprint $table) {
            ...
            $table->timestamps();
            $table->softDeletes();
            $table->foreignId(ReasonCode::ATTR_INT_CREATED_BY)->constrained(User::ATTR_TABLE)->onDelete('SET NULL');
            $table->foreignId(ReasonCode::ATTR_INT_UPDATED_BY)->constrained(User::ATTR_TABLE)->onDelete('SET NULL');
        });
    }
